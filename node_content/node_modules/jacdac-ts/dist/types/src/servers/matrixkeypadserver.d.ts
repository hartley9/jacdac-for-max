import { JDRegisterServer } from "../jdom/servers/registerserver";
import { SensorServer } from "./sensorserver";
export declare class MatrixKeypadServer extends SensorServer<[[number][]]> {
    readonly rows: JDRegisterServer<[number]>;
    readonly columns: JDRegisterServer<[number]>;
    readonly labels: JDRegisterServer<[[string][]]>;
    constructor(columns: number, rows: number, labels?: string[]);
    down(button: number): Promise<void>;
    up(button: number): Promise<void>;
}
//# sourceMappingURL=matrixkeypadserver.d.ts.map