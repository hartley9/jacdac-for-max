import { SystemStatusCodes } from "../../../jacdac-spec/dist/specconstants";
 ...
 * @category Servers
 */
export declare class JDServiceServer extends JDEventSource {
    readonly serviceClass: number;
    serviceIndex: number;
    private _device;
    readonly specification: jdspec.ServiceSpec;
    ...
    private _twin;
    private _twinCleanup;
    private _locked;
    constructor(serviceClass: number, options?: JDServerOptions);
    get device(): JDServerServiceProvider;
    set device(value: JDServerServiceProvider);
    get twin(): JDService;
    set twin(service: JDService);
    private handleTwinPacket;
    get registers(): JDRegisterServer<PackedValues>[];
    get timestamp(): number;
    register<TValues extends PackedValues = PackedValues>(code: number): JDRegisterServer<TValues>;
    protected addExistingRegister<TValues extends PackedValues = PackedValues>(reg: JDRegisterServer<TValues>): JDRegisterServer<TValues>;
    protected addRegister<TValues extends PackedValues = PackedValues>(identifier: number, defaultValue?: TValues): JDRegisterServer<TValues>;
    reset(): void;
    /**
     * Locks the current set of registers
     */
    lock(): void;
    protected addCommand(identifier: number, handler: (pkt: Packet) => void): void;
    handlePacket(pkt: Packet): Promise<void>;
    sendPacketAsync(pkt: Packet): Promise<void>;
    sendEvent(eventCode: number, data?: Uint8Array): Promise<void>;
    private handleCalibrate;
    calibrate(): Promise<void>;
}
export default JDServiceServer;
//# sourceMappingURL=serviceserver.d.ts.map